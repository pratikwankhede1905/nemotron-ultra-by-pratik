...
// The frontend is served by this same Express application.
// This fixes "Cannot GET /" when deployed as a single Render Web Service.
const publicDir = path.join(__dirname, "..", "public");
app.use(express.static(publicDir));

const configuredOrigin = process.env.FRONTEND_ORIGIN;
app.use(cors({
  origin: configuredOrigin ? configuredOrigin.split(",").map(x => x.trim()) : true
}));
...
// Explicit root route. Express static also serves this, but this makes
// the deployment behavior unambiguous.
app.get("/", (req, res) => {
  res.sendFile(path.join(publicDir, "index.html"));
});

app.get("/health", (req, res) => {
  res.json({ ok: true, service: "nemotron-ultra-backend" });
});
